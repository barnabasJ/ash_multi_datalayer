defmodule TodoClient.Test.CountingRouter do
  @moduledoc """
  Wraps the backend's RPC router, counting every `/rpc/run` request — the
  wire-level ground truth for "this read did not touch the server".
  """
  ...
  def init(opts), do: TodoServer.RpcRouter.init(opts)

  def call(%Plug.Conn{request_path: "/rpc/run"} = conn, opts) do
    :counters.add(:persistent_term.get(@counter_key), 1, 1)
    TodoServer.RpcRouter.call(conn, opts)
  end

  def call(conn, opts), do: TodoServer.RpcRouter.call(conn, opts)
end
